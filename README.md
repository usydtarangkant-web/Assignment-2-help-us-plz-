...
### Notes
Both boards must use the same baud rate. TX connects to RX and ground must be shared. Packet framing improves reliable communication. Interrupt-based reception reduces CPU polling and improves responsiveness.
...
### Notes
Servo movement is scaled from 0–360° heading into the servo pulse range, using magnetometer readings. UART baud rate must match on both boards. TX connects to RX and ground must be common. This design demonstrates modular embedded software integration using interrupts, communication, sensing, and actuation.
